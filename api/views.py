...
#    return HttpResponse(content, content_type="application/json")
    Result = {'status': 'Successfully uploaded', 'result_url': request.build_absolute_uri('/api/result?id=' + asyncResult.id), 'url_text': 'Proceed to result',}
...
def debug(request):
    ...
    if request.method == "POST":
        if not request.FILES.has_key('file'):
            ...
            tmpfiledir = os.path.dirname(os.path.abspath(__file__)).encode('utf-8')
            savedReplayFile = NamedTemporaryFile(prefix=tmpfiledir, delete=False)
